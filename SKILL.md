...
The install script uses the `lh` CLI (LobeHub / TripYoYo) to create the agents in your workspace. Install `lh` first if you haven't.

### Use

In your main chat topic, orchestrate the pipeline:
...
See `workflows/standard-5-stage.md` for the full playbook and `examples/rss-recommendation-system.md` for a real case study.
...
model: claude-opus-4-7      # or claude-sonnet-4-6, gpt-5, etc.
provider: ada               # or tripflow, openai, anthropic, etc.
plugins:
  - lobe-artifacts
